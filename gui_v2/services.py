...
def _set_isrc(recording, value):
    current = recording.identifiers.filter(
...
def save_release_track_rows(*, release, rows):
    ...
    context = (
        suppress_automatic_flac_sync()
        if suppress_automatic_flac_sync
        else nullcontext()
    )
    with context:
        recording_ids = set(
        ...
        if len(submitted_sequences) != len(set(submitted_sequences)):
            raise ValidationError(
                "Sorteringsrekkefølge må være unik innen utgivelsen."
            )
        ...
        for row in rows:
            ...
            if row.get("remove"):
                if track:
                    linked_files = list(
                        track.file_assets.values_list("filename", flat=True)[
                            :4
                        ]
                    )
                    if linked_files:
                        filenames = ", ".join(
                            f"«{name}»" for name in linked_files
                        )
            ...
            recording = (
                Recording.objects.filter(pk=recording_id).first()
                if recording_id
                else None
            )
            ...
            if is_new_recording or row.get("update_shared_recording"):
                if row.get("update_shared_recording") and row.get(
                    "recording_title"
                ):
                ...
                _replace_credits(
                    recording,
                    RecordingContribution.Role.COMPOSER,
                    row.get("composers"),
                )
                _replace_credits(
                    recording,
                    RecordingContribution.Role.LYRICIST,
                    row.get("lyricists"),
                )
                _replace_credits(
                    recording,
                    RecordingContribution.Role.ARRANGER,
                    row.get("arrangers"),
                )
            saved.append(track)
        return saved
