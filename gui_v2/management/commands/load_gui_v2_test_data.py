...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        riks, _ = Channel.objects.get_or_create(
            code="p7-riks", defaults={"name": "P7 Riks"}
        )
        ung, _ = Channel.objects.get_or_create(
            code="p7-ung", defaults={"name": "P7 Ung"}
        )
        voksen, _ = TargetAudience.objects.get_or_create(
            code="voksen", defaults={"name": "Voksen"}
        )
        ung_voksen, _ = TargetAudience.objects.get_or_create(
            code="ung-voksen", defaults={"name": "Ung voksen"}
        )
        label, _ = Label.objects.get_or_create(name="Nordlys Demo")
        release_a, _ = Release.objects.get_or_create(
            title="Nordlysarkivet",
        ...
        release_b, _ = Release.objects.get_or_create(
            title="Signaler fra nord",
            catalogue_number="DEMO-002",
            defaults={
        ...
        for index, (
            ...
            party, _ = Party.objects.get_or_create(
                name=artist_name, kind=Party.Kind.GROUP
            )
            identity, _ = ArtistIdentity.objects.get_or_create(
                party=party, display_name=artist_name
            )
            recording, _ = Recording.objects.get_or_create(
                title=title,
                defaults={
                    "duration_ms": (155 + index * 7) * 1000,
                    "metadata_status": Recording.Status.REVIEWED,
                },
            )
            RecordingContribution.objects.get_or_create(
                ...
                defaults={
                    ...
                    "credited_as": artist_name,
                },
            )
            ExternalIdentifier.objects.get_or_create(
                scheme=ExternalIdentifier.Scheme.ISRC,
                namespace="",
                normalized_value=f"NOZ9A26{index:05d}",
                defaults={
                    "recording": recording,
                    "value": f"NO-Z9A-26-{index:05d}",
                },
            )
            ...
            for channel in (riks, *([ung] if index % 3 == 0 else [])):
                MusicLibraryChannel.objects.get_or_create(
                    library_entry=entry, channel=channel
                )
            for audience in (
                voksen,
                *([ung_voksen] if index % 2 == 0 else []),
            ):
            ...
            if index in {1, 2, 5}:
                ManagedRecording.objects.get_or_create(
                    library_entry=entry,
                    defaults={"status": ManagedRecording.Status.ACTIVE},
                )
            release = releases[(index - 1) // 6]
            track, _ = ReleaseTrack.objects.get_or_create(
                release=release,
                sequence_number=((index - 1) % 6) + 1,
                defaults={
                    "recording": recording,
                    "disc_number": 1,
                    "side": "A" if index % 6 < 3 else "B",
                    "track_number": ((index - 1) % 3) + 1,
                    "duration_ms": recording.duration_ms,
                },
            )
            asset, _ = FileAsset.objects.get_or_create(
                recording=recording,
                filename=f"{index:02d}-{title.lower().replace(' ', '-')}.flac",
                defaults={
                    "role": FileAsset.Role.RADIO_FLAC,
                    "mime_type": "audio/flac",
                    "size_bytes": 1000000 + index,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "GUI v2-testdata er klare: 12 oppdiktede innspillinger og 2 utgivelser."
            )
        )
